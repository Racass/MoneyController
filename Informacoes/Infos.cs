...
namespace MoneyController
{
    public partial class Infos : Form
    {
        ...
        {
            int divisor = GetDifDias(DateTime.Now, Convert.ToDateTime(@"30/" + DateTime.Now.Month.ToString() + @"/" + DateTime.Now.Year.ToString()));
            if(divisor != 0)
        ...
        private void CalcRDebit(int divisor)
        {
            ...
            while (i < listagem.Count)
            {
                ...
                {
                    if (listagem[i].PagRec.Replace(" ", "") == "Recebimento")
        ...
        private void CalcAllDebit()
        {
            string initial;
            double total = 0;
            initial = DateTime.Now.ToShortDateString();
            total = 30 - Convert.ToDouble(initial.Substring(0, 2));
            total = Convert.ToDouble(din.Text) / total;
            RsDay.Text = total.ToString();
        }
        private void CalcDebit()
        {
            ...
            while( i < listagem.Count)
            {
                ...
                else if (listagem[i].DinCard.Replace(" ", "") == "Poup")
                {
                    if (listagem[i].PagRec.Replace(" ", "") == "Recebimento")
            ...
            SaldoTotal.Text = Convert.ToString(Convert.ToDouble(poupanc.Text) + Convert.ToDouble(din.Text));
        }
        ...
        {
            Informacoes.PorLocal ChamarCalc = new Informacoes.PorLocal();
            ChamarCalc.ShowDialog(this);
        }

        private void PagFixo_Click(object sender, EventArgs e)
        {
            Informacoes.PagFixo pagForm = new Informacoes.PagFixo();
            pagForm.ShowDialog(this);
        }
    }
}
